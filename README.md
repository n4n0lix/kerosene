...
### 2.3 pointers
raw pointers are strictly forbidden! use either `owner<T>/weak<T>` for representing single ownership or `shared_ptr<T>/weak_ptr<T>` for multiple ownership. Every object, unless a 3rd library promises to manage the life time of a raw pointer, has it's lifetime to be managed by a smart pointer.
